    ...
    <PaperContent>
      {{#if @controller.monthReimbursementCollections}}
        <PaperDataTable @class='reimbursements layout-row' as |table|>
          ...
          <table.body as |body|>
            {{#each
              @controller.monthReimbursementCollections
              as |monthReimbursementCollection|
            }}
              <body.row as |row|>
                ...
                <row.cell>
                  <CopyButton
                    @text={{monthReimbursementCollection.clipboardText}}
                  >
                    {{paperIcon
                      'content copy'
              ...
              {{#each
                monthReimbursementCollection.reimbursementCollections
                as |reimbursementCollection|
              }}
                {{#if reimbursementCollection.reimbursements}}
                  <body.row @class='person' as |row|>
                    ...
                    <row.cell>
                      <CopyButton
                        @text={{reimbursementCollection.clipboardText}}
                      >
                        {{paperIcon
                          'content copy'
      ...
      <PaperSwitch
        ...
        @value={{@controller.showProcessed}}
        @onChange={{fn (mut @controller.showProcessed)}}
      >
            ...
            {{#each @controller.processedReimbursements as |reimbursement|}}
              <body.row @class='reimbursement' as |row|>
                ...
                <row.cell @class='ride'>
                  {{#if
                    (and
                      reimbursement.ride.start
                      reimbursement.ride.institution.name
                    )
                  }}
                    {{momentFormat reimbursement.ride.start 'YYYY-MM-DD'}}
                    to
                    {{reimbursement.ride.institution.name}}
                  {{/if}}
                </row.cell>
