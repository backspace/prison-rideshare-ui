      ...
      <section>
        {{#if @controller.viewingSlot}}
          <div class='viewing-slot'>
            ...
            <PaperChips
              @removeItem={{@controller.deleteCommitment}}
              @addItem={{@controller.createCommitment}}
              @placeholder='Commit someone to this slot'
              @content={{@controller.viewingSlot.commitments}}
              @options={{@controller.uncommittedPeople}}
              @class='commitments'
              @searchField='name'
              as |person_or_commitment|
            >
        ...
        <PaperButton
          ...
          @raised={{true}}
          @onClick={{@controller.addAllActive}}
        />
        <PaperChips
          @removeItem={{@controller.removePerson}}
          @addItem={{@controller.addPerson}}
          @placeholder='Add a person to email'
          @content={{@controller.people}}
          @options={{@controller.remainingPeople}}
          @searchField='name'
          as |person|
        >
          {{person.name}}
        ...
        <PaperButton
          @primary={{gt @controller.people.length 0}}
          @raised={{gt @controller.people.length 0}}
          @onClick={{@controller.email}}
        >
          Email
          {{@controller.monthString}}
          calendar link
        </PaperButton>
